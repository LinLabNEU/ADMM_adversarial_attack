...
from l2_ADMM_attack import ADMML2
from li_ADMM_attack import ADMMLi
from l1_ADMM_attack import ADMML1
...
def main(args):
    with tf.Session() as sess:
        ...
        inputs, targets, labels, true_ids = generate_data(data, model, samples=args['numimg'], targeted=True,
                                                          start=0, inception=inception, handpick=handpick,
                                                          seed=args['seed'])
        ...
        if args['attack'] == 'L0A':
            attack = ADMML0(sess, model, batch_size=args['batch_size'], max_iterations=args['maxiter'],
                            confidence=args['conf'], binary_search_steps=args['binary_steps'],
                            in_ro=args['ro'], iteration_steps=args['iteration_steps'], inception=inception,
                            abort_early=args['abort_early'])

        if args['attack'] == 'LiA':
            attack = ADMMLi(sess, model, batch_size=args['batch_size'], max_iterations=args['maxiter'],
                            confidence=args['conf'], binary_search_steps=args['iteration_steps'], ro=args['ro'],
                            abort_early=args['abort_early'])

        if args['attack'] == 'L1A':
            attack = ADMML1(sess, model, batch_size=args['batch_size'], max_iterations=args['maxiter'],
                            confidence=args['conf'], binary_search_steps=args['iteration_steps'],
                            ro=args['ro'], abort_early=args['abort_early'])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-is", "--iteration_steps", type=int, default=10, help="number of iteration")
    parser.add_argument("-ro", "--ro", type=int, default=20, help="value of ro")
    ...
    parser.add_argument("-sd", "--seed", type=int, default=96,
    ...
    parser.add_argument("-adv", "--adversarial", choices=["none", "l2", "l1", "en"], default="none",
